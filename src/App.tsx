              ...
              {fields.map(({ key, label, prefix }) => (
                <label key={key} className="block text-sm font-medium text-slate-700">
                  ...
                  <div className="relative mt-1.5">
                    ...
                    <input type="number" min="0" step="any" value={workload[key]} onChange={(event) => updateWorkload(key, event.target.value)} className={`w-full rounded-md border border-slate-300 py-2 text-slate-900 outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-100 ${prefix ? "pl-7 pr-3" : "px-3"}`} />
                  </div>
                </label>
              ))}
            </div>
